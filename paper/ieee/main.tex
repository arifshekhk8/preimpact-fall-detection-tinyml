...
\begin{figure*}[t]
\centering
\resizebox{\textwidth}{!}{\input{fig_method}}
\caption{The deployed system. \textbf{Top:} the on-device pipeline. Sampling is driven
by a hardware timer rather than a polled delay, because loop jitter accumulates and
shifts window content away from the training distribution. \textbf{The red boundary is
the point of the figure:} per-window instance normalisation is performed in float
\emph{before} quantisation rather than as a layer inside the graph. The two are
mathematically identical in FP32 and differ by 30.95 points of macro-F1 under INT8
(Section~\ref{sec:c3}). \textbf{Bottom:} the network. Separable convolutions and
global average pooling hold the model to 7{,}947 parameters.}
\label{fig:method}
\end{figure*}
...
\begin{table}[t]
\caption{Deployment on ESP32 DevKit V1 with MPU6050. The deployed model is trained on
SisFall, KFall and FallAllD and evaluated on held-out subjects.}
\label{tab:four}
\centering\small
\begin{tabular}{lrr}
\toprule
...
free-living wear; and the hardware cells of Table~\ref{tab:four} remain to be
measured.
